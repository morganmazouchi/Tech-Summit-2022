...
# MAGIC 6. To refresh landing zone, run Cmd 6
...
output_path = dbutils.widgets.get('StoragePath')
print ("output_path:", output_path)

# schema_output_path = dbutils.widgets.get('Storage_schemaPath')
# ref_output_path = dbutils.widgets.get('Storage_ref_Path')
ref_output_path = "{}".format(output_path + "/ref_accounting_treatment")
schema_output_path = "{}".format(output_path + "/account_schema.json")
print ("ref_output_path:", ref_output_path)

dbutils.fs.rm(schema_output_path, True)
dbutils.fs.rm(ref_output_path, True)
...
spark.createDataFrame([
  ...
  (4, 'held_for_hedge'),
  (5, 'fv_designated')
], ['id', 'accounting_treatment']).write.format('delta').save(ref_output_path)
...
for i in range(0, 7):
  time.sleep(int(dbutils.widgets.get('Speed')))
  write_batches_to_file(generate_loans("account", int(dbutils.widgets.get('Volume'))), f'/dbfs{output_path}/landing/accounts{i}.json')
